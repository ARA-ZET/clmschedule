...
class JobAssignmentService {
  ...
  List<Distributor> findAvailableDistributors(DateTime date) {
    ...
    final assignedDistributorIds = jobsForDate.map((job) => job.distributorId).toSet();
    
    // Filter out assigned distributors and sort by index
    final availableDistributors = allDistributors
        .where((distributor) => !assignedDistributorIds.contains(distributor.id))
        .toList();
  ...
  List<JobAssignment> calculateJobAssignments({
    ...
    while (remainingDays > 0) {
      ...
      final distributorsToAssign = availableDistributors.take(remainingDays).toList();
  ...
  List<Job> createJobsFromAssignments(List<JobAssignment> assignments) {
    return assignments.map((assignment) => Job(
  ...
  Future<List<Job>> createJobAssignmentsFromJobList(JobListItem jobListItem) async {
